...
export const BaseSection: React.FC<BaseSectionComponentProps> = ({
  ...
  const onSubmit = async (data: Record<string, any>) => {
    ...
    try {
      ...
      if (step === 'team') {
        updateResponses({ team: data } as Partial<AssessmentResponses>);
      } else if (step === 'social-media') {
        updateResponses({ socialMedia: data } as Partial<AssessmentResponses>);
      } else {
  ...
  if (isMobile) {
    return (
      <div className="w-full px-4 py-6 space-y-8 min-h-screen bg-white">
